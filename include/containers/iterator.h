...
#define FOR_RANGE(it, container)                                                                            \
    Iterator *__DECL(__LINE__) __attribute__((cleanup(cleanup_iterator))) = container->begin(container);    \
    for (it = __DECL(__LINE__); ((struct __internal_iterator_s *)it)->__index < ((struct __internal_iterator_s *)it)->__size; ((struct __internal_iterator_s *)it)->__index++)
